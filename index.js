...
const invocationName = "friendly quiz";
...
const friends=[
  {
    ...
    option1: 'Option A: Insomnia Cafe',
    option2: 'Option B: central perk'
  }];
...
const answerIntent_Handler =  {
    ...
    handle(handlerInput) {
        ...
        //   SLOT: option 
        if (slotValues.option.heardAs && slotValues.option.heardAs !== '') {
            slotStatus += ' slot option was heard as ' + slotValues.option.heardAs + '. ';
        } else {
            slotStatus += 'slot option is empty. ';
        }
        if (slotValues.option.ERstatus === 'ER_SUCCESS_MATCH') {
            slotStatus += 'a valid ';
            if(slotValues.option.resolved !== slotValues.option.heardAs) {
                slotStatus += 'synonym for ' + slotValues.option.resolved + '. '; 
                } else {
                slotStatus += 'match. '
            } // else {
                //
        }
        if (slotValues.option.ERstatus === 'ER_SUCCESS_NO_MATCH') {
            slotStatus += 'which did not match any slot value. ';
            console.log('***** consider adding "' + slotValues.option.heardAs + '" to the custom slot type used by slot option! '); 
        }

        if( (slotValues.option.ERstatus === 'ER_SUCCESS_NO_MATCH') ||  (!slotValues.option.heardAs) ) {
           // slotStatus += 'A few valid values are, ' + sayArray(getExampleSlotValues('answerIntent','option'), 'or');
        }
...
const decisionIntent_Handler =  {
    canHandle(handlerInput) {
        const request = handlerInput.requestEnvelope.request;
        return request.type === 'IntentRequest' && request.intent.name === 'decisionIntent' ;
    },
    handle(handlerInput) {
        const request = handlerInput.requestEnvelope.request;
        const responseBuilder = handlerInput.responseBuilder;
        ...
        let decvar=getSlotValues(request.intent.slots);
        let dec=decvar.decision.heardAs;
        
        if(dec=='no' || dec=='naah' || dec=='nope')
        {
          say+='Is that what a dinosaur would do?';
        }
    ...
    "languageModel": {
      ...
      "intents": [
        ...
        },
        {
          ...
          "samples": [
            "Is the answer option {option}",
            "Answer is option {option}",
            "I think it is option {option}",
            "Option {option}",
            "The answer is option {option}",
            "Can the answer be {answer}",
            "Answer is {answer}",
